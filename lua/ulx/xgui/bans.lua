...
local searchFilter = ""
xbans.searchbox = xlib.maketextbox { x = 5, y = 6, w = 175, text = "搜索...", selectall = true, parent = xbans }
...
local sortMode = 0
local sortAsc = false
xbans.sortbox = xlib.makecombobox { x = 185, y = 6, w = 150, text = "排序: 日期 (降序.)", choices = { "日期", "名字", "Steam ID", "管理员", "原因", "解封日期", "封禁时长" }, parent = xbans }
function xbans.sortbox:OnSelect(i, v)
    ...
    self:SetValue("排序: " .. v .. (sortAsc and " (升序.)" or " (降序.)"))
    xbans.setPage(1)
    xbans.retrieveBans()
end

local hidePerma = 0
xlib.makebutton { x = 335, y = 6, w = 95, label = "永久封禁: 全部", parent = xbans }.DoClick = function(self)
    hidePerma = hidePerma + 1
    if hidePerma == 1 then
        self:SetText("永久封禁: 隐藏")
    elseif hidePerma == 2 then
        self:SetText("永久封禁: 仅显示")
    elseif hidePerma == 3 then
        hidePerma = 0
        self:SetText("永久封禁: 全部")
    end
    xbans.setPage(1)
    xbans.retrieveBans()
end
...
xlib.makebutton { x = 430, y = 6, w = 120, label = "未完成封禁: 显示", parent = xbans, tooltip =
"过滤器由ULib提供支持, 但是ULib不拥有任何数据." }.DoClick = function(self)
    hideIncomplete = hideIncomplete + 1
    if hideIncomplete == 1 then
        self:SetText("未完成封禁: 隐藏")
    elseif hideIncomplete == 2 then
        self:SetText("未完成封禁: 仅显示")
    elseif hideIncomplete == 3 then
        hideIncomplete = 0
        self:SetText("未完成封禁: 显示")
    end
    xbans.setPage(1)
    xbans.retrieveBans()
end
...
local function banUserList(doFreeze)
    ...
    menu:AddSpacer()
    if LocalPlayer():query("ulx banid") then menu:AddOption("使用STEAMID...", function() xgui.ShowBanWindow() end) end
    menu:Open()
end

xlib.makebutton { x = 5, y = 340, w = 70, label = "封禁...", parent = xbans }.DoClick = function() banUserList(false) end
xbans.btnFreezeBan = xlib.makebutton { x = 80, y = 340, w = 95, label = "冻结封禁...", parent = xbans }
xbans.btnFreezeBan.DoClick = function() banUserList(true) end

xbans.infoLabel = xlib.makelabel { x = 204, y = 344, label = "右键一个封禁打开选项", parent = xbans }


xbans.resultCount = xlib.makelabel { y = 344, parent = xbans }
function xbans.setResultCount(count)
    local pnl = xbans.resultCount
    pnl:SetText(count .. " 个结果")
    pnl:SizeToContents()
...
local numPages = 1
local pageNumber = 1
xbans.pgleft = xlib.makebutton { x = 480, y = 340, w = 20, icon = "icon16/arrow_left.png", centericon = true, disabled = true, parent =
    xbans }
...
xbans.pgright = xlib.makebutton { x = 557, y = 340, w = 20, icon = "icon16/arrow_right.png", centericon = true, disabled = true, parent =
    xbans }
...
function xbans.RemoveBan(ID, bandata)
    local tempstr = "<未知>"
    if bandata then tempstr = bandata.name or "<未知>" end
    Derma_Query("你确定你想要解封 " .. tempstr .. " - " .. ID .. "?", "XGUI 警告",
        "移除", function()
...
function xbans.ShowBanDetailsWindow(bandata)
    ...
    xbans.openWindows[bandata.steamID] = xlib.makeframe { label = "封禁详细", x = wx, y = wy, w = 285, h = 295, skin =
        xgui.settings.skin }
    ...
    if bandata.admin then
        xlib.makelabel { x = 90, y = 165, label = string.match(bandata.admin, "%(STEAM_%w:%w:%w*%)"), parent = panel }
    end
    ...
    if bandata.modified_admin then
        xlib.makelabel { x = 90, y = 240, label = string.match(bandata.modified_admin, "%(STEAM_%w:%w:%w*%)"), parent = panel }
    end

    panel.data = bandata -- Store data on panel for future reference.
    xlib.makebutton { x = 5, y = 265, w = 89, label = "编辑封禁...", parent = panel }.DoClick = function()
        xgui.ShowBanWindow(nil, panel.data.steamID, nil, true, panel.data)
    end

    xlib.makebutton { x = 99, y = 265, w = 89, label = "解封", parent = panel }.DoClick = function()
        xbans.RemoveBan(panel.data.steamID, panel.data)
    end

    xlib.makebutton { x = 192, y = 265, w = 88, label = "关闭", parent = panel }.DoClick = function()
...
function xgui.ConvertTime(seconds)
    ...
    local curtime = ""
    if years ~= 0 then curtime = curtime .. years .. " 年" .. ((years > 1) and ", " or ", ") end
    if weeks ~= 0 then curtime = curtime .. weeks .. " 周" .. ((weeks > 1) and ", " or ", ") end
    if days ~= 0 then curtime = curtime .. days .. " 天" .. ((days > 1) and ", " or ", ") end
