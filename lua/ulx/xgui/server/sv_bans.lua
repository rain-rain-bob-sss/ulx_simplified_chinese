...
function bans.init()
    ...
    function bans.updateBan(ply, args)
        ...
        if not access then
            ULib.tsayError(ply, "错误编辑封禁:您必须有权访问 ulx 封禁, " .. ply:Nick() .. "!", true)
            return
        end
        ...
        if not ULib.isValidSteamID(steamID) then
            ULib.tsayError(ply, "无效的steamid", true)
            return
        end
        ...
        if not success then
            ULib.tsayError(ply, "错误编辑封禁: " .. err, true)
            return
        end
        ...
        if not success then
            ULib.tsayError(ply, "错误编辑封禁:您没有指定有效原因, " .. ply:Nick() .. "!", true)
            return
        end
    ...
    ulx.addToHelpManually("Menus", "xgui fban", "打开加禁窗口,冻结指定玩家,自动填写Name/SteamID. (say: !fban)")
    ulx.addToHelpManually("Menus", "xgui xban", "如果指定了玩家,则打开添加禁用窗口并自动填写名称/SteamID. (say: !xban)")
end
