...
function groups.list:populate()
    ...
    if prev_sel == "" then prev_sel = "选择权限组..." end
    self:Clear()
...
groups.aplayer = xlib.makebutton { x = 5, y = 210, w = 80, label = "添加...", parent = groups.pnlG1 }
...
groups.cplayer.DoClick = function()
    if groups.players:GetSelectedLine() then
        ...
        menu:AddSpacer()
        menu:AddOption("移除用户", function() groups.changeUserGroup(ID, "user") end)
        menu:Open()
    end
end
xlib.makelabel { x = 5, y = 240, label = "队伍:", parent = groups.pnlG1 }
...
groups.teams.OnSelect = function(self, index, value, data)
    if value == "<无>" then value = "" end
    RunConsoleCommand("xgui", "changeGroupTeam", groups.list:GetValue(), value)
end
groups.teambutton = xlib.makebutton { x = 5, y = 275, w = 160, label = "管理队伍 >>", disabled = not ulx
    .uteamEnabled(), parent = groups.pnlG1 }
...
groups.accessbutton = xlib.makebutton { x = 5, y = 305, w = 160, label = "管理权限 >>", parent = groups.pnlG1 }
...
function groups.addBySteamID(group)
    local frame = xlib.makeframe { label = "添加ID至组 " .. group, w = 190, h = 60, skin = xgui.settings.skin }
    xlib.maketextbox { x = 5, y = 30, w = 180, parent = frame, selectall = true, text = "输入STEAMID..." }.OnEnter = function(
        self)
        if ULib.isValidSteamID(self:GetValue()) then
            RunConsoleCommand("ulx", "adduserid", self:GetValue(), group)
            frame:Remove()
        else
            Derma_Message("无效的SteamID!", "XGUI 提示")
        end
    end
end
...
groups.newgroup = xlib.makebutton { x = 245, y = 175, w = 100, label = "新建组...", parent = groups.pnlG2 }
groups.newgroup.DoClick = function()
    if not ULib.ucl.groups[groups.gname:GetValue()] then
        ...
        Derma_Message("相同名称的组已存在!", "XGUI 提示");
    end
end
xlib.makelabel { x = 145, y = 8, label = "名字:", parent = groups.pnlG2 }
xlib.makelabel { x = 145, y = 33, label = "继承于:", parent = groups.pnlG2 }
xlib.makelabel { x = 145, y = 58, label = "可针对:", parent = groups.pnlG2 }
groups.gname = xlib.maketextbox { x = 205, y = 5, w = 140, text = "new_group", selectall = true, parent = groups.pnlG2 }
groups.ginherit = xlib.makecombobox { x = 205, y = 30, w = 140, text = "user", parent = groups.pnlG2 }
groups.gcantarget = xlib.maketextbox { x = 205, y = 55, w = 140, text = "", selectall = true, parent = groups.pnlG2 }
groups.gupdate = xlib.makebutton { x = 140, y = 175, w = 100, disabled = true, label = "更新", parent = groups.pnlG2 }
groups.gupdate.DoClick = function(self)
    ...
    if groups.gname:GetValue() ~= groupname then
        if groupname == "superadmin" or groupname == "admin" then
            Derma_Query("重命名组 " .. groupname .. " 可能不是一个好主意, 并且可能使一些插件发生问题. 你确定吗?",
                "XGUI 警告",
                "重命名至 " .. groups.gname:GetValue(), function()
...
groups.gdelete = xlib.makebutton { x = 5, y = 175, w = 130, label = "删除权限组", disabled = true, parent = groups
    .pnlG2 }
groups.gdelete.DoClick = function()
    local group = groups.gname:GetValue()
    if group == "superadmin" or group == "admin" then
        Derma_Query("移除组 " .. group .. " 可能不是一个好主意, 并且可能使一些插件发生问题. 你确定吗?",
            "XGUI 警告",
            "移除", function()
        ...
        Derma_Query("你确定要移除组 \"" .. group .. "\" ?", "XGUI 警告",
            "移除", function()
                RunConsoleCommand("ulx", "removegroup", group)
            end,
            "取消", function() end)
...
function groups.pnlG3:Open()
    if groups.pnlG4:IsVisible() then
        groups.accessbutton:SetText("管理权限 >>")
        groups.pnlG4:Close()
    end
    self:openAnim()
end
...
xlib.makebutton { x = 5, y = 160, w = 80, label = "新建队伍", parent = groups.pnlG3 }.DoClick = function()
...
groups.teamdelete.DoClick = function()
    ...
    Derma_Query("你确定想要移除队伍 \"" .. team .. "\" ?", "XGUI 警告",
        "移除", function() RunConsoleCommand("xgui", "removeTeam", team) end,
        "取消", function() end)
end
groups.upbtn = xlib.makebutton { x = 85, y = 160, w = 20, icon = "icon16/bullet_arrow_up.png", centericon = true, disabled = true, parent =
    groups.pnlG3 }
...
groups.downbtn = xlib.makebutton { x = 85, y = 180, w = 20, icon = "icon16/bullet_arrow_down.png", centericon = true, disabled = true, parent =
    groups.pnlG3 }
...
groups.teammodifiers:AddColumn("参数").DoClick = function() end
groups.teammodifiers:AddColumn("值").DoClick = function() end
groups.teammodifiers.OnRowSelected = function(self, LineID, Line)
    ...
    if Line:GetColumnText(3) ~= "number" then
        if Line:GetColumnText(1) == "name" then
            groups.teamctrl = xlib.maketextbox { selectall = true, text = Line:GetColumnText(2), parent = groups
                .teammodspace }
        ...
        groups.teamctrl = xlib.makeslider { min = defvalues[2] or 0, max = defvalues[3] or 2000, decimal = defvalues[4], value =
            tonumber(Line:GetColumnText(2)), label = Line:GetColumnText(1), parent = groups.teammodspace }
        groups.teamctrl.OnEnter = function(self)
            applybtn.DoClick()
        end
        groups.teammodspace:Add(groups.teamctrl)
...
groups.teammodadd = xlib.makebutton { x = 110, y = 180, w = 75, label = "添加..", disabled = true, parent = groups
    .pnlG3 }
...
groups.teammodremove = xlib.makebutton { x = 185, y = 180, w = 75, label = "移除", disabled = true, parent = groups
    .pnlG3 }
...
function groups.pnlG4:Open()
    if groups.pnlG3:IsVisible() then
        groups.teambutton:SetText("管理队伍 >>")
        groups.pnlG3:Close()
    end
    self:openAnim()
...
xlib.makelabel { x = 5, y = 5, label = "能够访问:", parent = groups.pnlG4 }
...
xlib.makelabel { x = 5, y = 5, label = "限制命令参数:", parent = groups.pnlG5 }
...
function groups.populateRestrictionArgs(cmd, accessStr)
        ...
        if not arg.type.invisible then
            ...
            if not arg.invisible then
                ...
                if arg.type == ULib.cmds.PlayerArg or arg.type == ULib.cmds.PlayersArg then
                    ...
                    outPanel.type = "ply"
                    outPanel.cantarget = xlib.makecheckbox { x = 5, y = 5, label = "无视 can_target", value =
                        ignoreCanTarget or 0, parent = outPanel }
                    outPanel.txtfield = xlib.maketextbox { x = 5, y = 25, w = 170, text = ignoreCanTarget and
                        string.sub(restrictions[argnum], 2) or restrictions[argnum] or "*", parent = outPanel }
                    ...
                    outCat = xlib.makecat { label = "限制 " .. (arg.hint or "玩家"), w = 180, checkbox = true, expanded =
                        hasrestriction, contents = outPanel, parent = xgui.null }
                    ---Number Argument---
                elseif arg.type == ULib.cmds.NumArg then
                    ...
                    if table.HasValue(arg, ULib.cmds.allowTimeString) then
                        ...
                        xlib.makelabel { x = 25, y = 8, label = "最小限制", parent = outPanel }
                        xlib.makelabel { x = 25, y = 48, label = "最大限制", parent = outPanel }
                    ...
                    outCat = xlib.makecat { label = "对 " .. (arg.hint or "数字参数") .. " 的限制", w = 180, checkbox = true, expanded =
                        hasrestriction, contents = outPanel, parent = xgui.null }
                    ---Bool Argument---
                elseif arg.type == ULib.cmds.BoolArg then
                    local outPanel = xlib.makepanel { h = 25 }
                    outPanel.type = "bool"
                    outPanel.checkbox = xlib.makecheckbox { x = 5, y = 5, value = restrictions[argnum] or false, label =
                    "Must be: True (1), False (0)", parent = outPanel }
                    outCat = xlib.makecat { label = "对 " .. (arg.hint or "开关参数") .. " 的限制", w = 180, checkbox = true, expanded =
                        hasrestriction, contents = outPanel, parent = xgui.null }
                    ---String Argument---
                elseif arg.type == ULib.cmds.StringArg then
                    local outPanel = xlib.makepanel { h = 200 }
                    outPanel.type = "str"
                    outPanel.list = xlib.makelistview { x = 5, y = 5, w = 170, h = 150, multiselect = false, parent =
                        outPanel }
                    outPanel.list:AddColumn("白名单内的字符串参数")
                        ...
                        if self:GetValue() ~= "" then
                            if not (string.find(self:GetValue(), "<") or string.find(self:GetValue(), ">") or string.find(self:GetValue(), ",")) then
                                ...
                                if not found then
                                    ...
                                    Derma_Message("该物品已存在于列表中!", "XGUI 提示")
                                end
                            else
                                Derma_Message("你不能使用带有以下字符的字符串: < > ,", "XGUI 提示")
                            end
                        end
                    end
                    outPanel.addButton = xlib.makebutton { x = 5, y = 175, w = 85, label = "添加", parent = outPanel }
                    outPanel.addButton.DoClick = function(self)
                        outPanel.textbox:OnEnter()
                    end
                    outPanel.removeButton = xlib.makebutton { x = 90, y = 175, w = 85, label = "移除", disabled = true, parent =
                        outPanel }
                    ...
                    outCat = xlib.makecat { label = "对 " .. (arg.hint or "字符串参数") .. " 的限制", w = 180, padding = 0, checkbox = true, expanded =
                        hasrestriction, contents = outPanel, parent = xgui.null }
                end
    ...
    groups.applyButton = xlib.makebutton { h = 20, label = "应用限制", parent = groups.rArgList }
    ...
    if not groups.access_lines[cmd].Columns[2]:GetChecked() then
        groups.applyButton:SetText("应用访问权限 + 限制")
    elseif groups.access_lines[cmd].Columns[2].disabled then
        ...
        groups.applyButton:SetText("应用访问权限 + 限制")
        groups.applyInheritedButton:SetText("应用对权限组 \"" ..
            groups.access_lines[cmd]:GetColumnText(4) .. "\" 的限制")
...
groups.pnlG3.openAnim = function(self)
    ...
    xlib.addToAnimQueue("pnlSlide",
        { panel = self, startx = -410, starty = 130, endx = 5, endy = 130, setvisible = true })
end
groups.pnlG3.closeAnim = function(self)
    xlib.addToAnimQueue("pnlSlide",
        { panel = self, startx = 5, starty = 130, endx = -410, endy = 130, setvisible = false })
