...
plist:Add(xlib.makelabel { label = "ULX 投票换图设置" })
plist:Add(xlib.makelabel { label = "要使切换地图投票成功所需的比例(百分比)" })
plist:Add(xlib.makeslider { label = "<--->", min = 0, max = 1, decimal = 2, repconvar = "ulx_votemap2Successratio" })
plist:Add(xlib.makelabel { label = "至少需要多少票才能成功" })
plist:Add(xlib.makeslider { label = "<--->", min = 0, max = 10, repconvar = "ulx_votemap2Minvotes" })
xgui.addSubModule("ULX 地图投票", plist, nil, "server")
...
function adverts.RenameAdvert(old)
    ...
    xlib.makebutton { x = 175, y = 55, w = 50, label = "确定", parent = advertRename }.DoClick = function()
...
xgui.addSubModule("ULX 公告", adverts, nil, "server")

---------------------------Ban Message---------------------------
xgui.prepareDataType("banmessage")
local plist = xlib.makelistlayout { w = 275, h = 322, parent = xgui.null }
plist:Add(xlib.makelabel { label = "显示给被封禁玩家的信息", zpos = 1 })
plist.txtBanMessage = xlib.maketextbox { zpos = 2, h = 236, multiline = true }
plist:Add(plist.txtBanMessage)
plist:Add(xlib.makelabel { label = "插入参数:", zpos = 3 })
plist.variablePicker = xlib.makecombobox { choices = { "封禁者 - Admin:SteamID 谁封禁了此玩家", "封禁日期 - 什么时候开始封禁的", "原因", "剩余时间", "SteamID (不包括非数字字符)", "SteamID64" }, zpos = 4 }
plist:Add(plist.variablePicker)

plist.btnPreview = xlib.makebutton { label = "预览信息", zpos = 4 }
...
plist.btnSave = xlib.makebutton { label = "保存信息", zpos = 5 }
...
xgui.addSubModule("ULX 封禁提示", plist, nil, "server")
...
xgui.addSubModule("ULX 胡言乱语设置", gimps, nil, "server")
...
xgui.addSubModule("ULX 踢出/封禁原因", panel, "xgui_managebans", "server")
...
xgui.addSubModule("ULX 日志设置", plist, nil, "server")
...
plist.generator:Add(xlib.makelabel { label = "MOTD 生成器_标题:", zpos = -2 })
...
local btnAddSection = xlib.makebutton { label = "添加新的分类..." }
btnAddSection.DoClick = function()
    local menu = DermaMenu()
    menu:SetSkin(xgui.settings.skin)
    menu:AddOption("小标题", function()
    ...
    menu:AddOption("小黑点列表", function()
        local info = xgui.data.motdsettings.info
        table.insert(info, {
            type = "list",
            title = "样例",
            contents = { "每一行文字的前面都会有小黑点.", "你可以尽可能地添加!" }
        })
        performMOTDInfoUpdate(info[#info], "info[" .. #info .. "]")
    end)
    menu:AddOption("数字列表", function()
        local info = xgui.data.motdsettings.info
        table.insert(info, {
            type = "ordered_list",
            title = "样本数字列表",
            contents = { "每一行字的前面都会有数字.", "你可以尽可能地添加!" }
        })
        performMOTDInfoUpdate(info[#info], "info[" .. #info .. "]")
    end)
    menu:AddOption("已安装的模组", function()
        local info = xgui.data.motdsettings.info
        table.insert(info, {
            type = "mods",
            title = "已安装的模组"
    ...
    menu:AddOption("服务器管理员", function()
        local info = xgui.data.motdsettings.info
        table.insert(info, {
            type = "admins",
            title = "我们的管理员",
...
plist.updateGeneratorSettings = function(data)
    ...
    for i = 1, #data.info do
        ...
        local btnRemove = xlib.makebutton { w = 100, label = "移除分类", parent = actionPnl }
        btnRemove.DoClick = function()
            Derma_Query("你确定要移除分类 \"" .. section.title .. "\"?", "XGUI 警告",
                "移除", function()
        ...
        local btnUp = xlib.makebutton { x = 214, w = 20, icon = "icon16/bullet_arrow_up.png", centericon = true, disabled = (i == 1), parent =
            actionPnl }
        ...
        local btnDown = xlib.makebutton { x = 234, w = 20, icon = "icon16/bullet_arrow_down.png", centericon = true, disabled = (i == #data.info), parent =
            actionPnl }
...
function plist.ConVarUpdated(sv_cvar, cl_cvar, ply, old_val, new_val)
    if string.lower(cl_cvar) == "ulx_showmotd" then
        ...
        if new_val == "0" then
            ...
            plist.lblDescription:SetText("MOTD 已被关闭.\n")
        elseif new_val == "1" then
            showMotdFile = true
            plist.lblDescription:SetText("MOTD 将会显示文件内容.\n根目录位于客户端/服务端主目录.\n")
        elseif new_val == "2" then
            showGenerator = true
            plist.lblDescription:SetText("MOTD 将基于生成器显示.\n")
        elseif new_val == "3" then
            showURL = true
            plist.lblDescription:SetText(
                "MOTD 将会显示URL内容.\n你可以使用 %curmap% 和 %steamid%\n(例如, server.com/?map=%curmap%&id=%steamid%)\n")
        end
...
xgui.addSubModule("ULX MOTD设置", motdpnl, "ulx showmotd", "server")

-----------------------Player Votemap List-----------------------
xgui.prepareDataType("votemaps", ulx.votemaps)
local panel = xlib.makepanel { w = 285, h = 322, parent = xgui.null }
xlib.makelabel { label = "允许投票的地图", x = 5, y = 3, parent = panel }
xlib.makelabel { label = "不允许投票的地图", x = 150, y = 3, parent = panel }
...
panel.remainingmaps = xlib.makelistview { x = 140, y = 20, w = 135, h = 262, multiselect = true, headerheight = 0, parent =
    panel }
...
panel.remove = xlib.makebutton { y = 282, w = 135, label = "移除 -->", disabled = true, parent = panel }
...
local plist = xlib.makelistlayout { w = 275, h = 322, parent = xgui.null }
plist:Add(xlib.makelabel { label = "玩家投票设置" })
plist:Add(xlib.makecheckbox { label = "启用votemap功能", repconvar = "ulx_votemapEnabled" })
plist:Add(xlib.makelabel { label = "每隔一次投票的冷却时间" })
plist:Add(xlib.makeslider { label = "<--->", min = 0, max = 300, repconvar = "ulx_votemapMintime" })
plist:Add(xlib.makelabel { label = "每隔一次更改投票的冷却时间" })
plist:Add(xlib.makeslider { label = "<--->", min = 0, max = 60, decimal = 1, repconvar = "ulx_votemapWaittime" })
plist:Add(xlib.makelabel { label = "投票成功所需比例(百分比)" })
...
plist:Add(xlib.makelabel { label = "管理员需要多长时间才能使用veto" })
plist:Add(xlib.makeslider { label = "<--->", min = 0, max = 300, repconvar = "ulx_votemapVetotime" })
xgui.addSubModule("ULX 玩家投票设置", plist, nil, "server")

-------------------------Reserved Slots--------------------------
local plist = xlib.makelistlayout { w = 275, h = 322, parent = xgui.null }
plist:Add(xlib.makelabel { label = "预留位置" })
plist:Add(xlib.makecombobox { repconvar = "ulx_rslotsMode", isNumberConvar = true, choices = { "0 - 已关闭",
    "1 - 留位置给管理员", "2 - 不留位置给管理员", "3 - 踢出连接时间最短玩家" } })
plist:Add(xlib.makeslider { label = "预留位置数量", min = 0, max = game.MaxPlayers(), repconvar = "ulx_rslots" })
plist:Add(xlib.makecheckbox { label = "预留位置不可见", repconvar = "ulx_rslotsVisible" })
plist:Add(xlib.makelabel { w = 265, wordwrap = true, label =
"帮助信息:\n1 - 设置一个可见的预留位置数量,\n当管理员加入时, 将会塞满这些位置.\n\n2 - 与#1一样, 但管理员不会塞满这些位置,他们\n将会在有玩家离开的时候可用.\n\n 3 - 总是保留一个位置给管理员,如果\n满了的话,就会踢掉连接服务器时间最短\n的玩家,继续保持1个位置可用.\n\n预留位置不可见:\n当启用时,如果没有位置可用的话,将会\n显示服务器已满.管理员将无法通过正常\n方式进入服务器.只能使用 'connect <ip>' 来\n连接到服务器" })
xgui.addSubModule("ULX 预留位置", plist, nil, "server")

------------------------Votekick/Voteban-------------------------
local plist = xlib.makelistlayout { w = 275, h = 322, parent = xgui.null }
plist:Add(xlib.makelabel { label = "投票踢出设置" })
plist:Add(xlib.makelabel { label = "踢出投票生效所需百分比" })
plist:Add(xlib.makeslider { label = "<--->", min = 0, max = 1, decimal = 2, repconvar = "ulx_votekickSuccessratio" })
plist:Add(xlib.makelabel { label = "踢出投票至少要多少票支持才生效" })
plist:Add(xlib.makeslider { label = "<--->", min = 0, max = 10, repconvar = "ulx_votekickMinvotes" })
plist:Add(xlib.makelabel { label = "\n投票封禁设置" })
plist:Add(xlib.makelabel { label = "封禁投票生效所需百分比" })
plist:Add(xlib.makeslider { label = "<--->", min = 0, max = 1, decimal = 2, repconvar = "ulx_votebanSuccessratio" })
plist:Add(xlib.makelabel { label = "封禁投票至少要多少票支持才生效" })
plist:Add(xlib.makeslider { label = "<--->", min = 0, max = 10, repconvar = "ulx_votebanMinvotes" })
xgui.addSubModule("ULX 投票踢出/封禁设置", plist, nil, "server")
