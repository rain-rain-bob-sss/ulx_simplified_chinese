...
function xgui.checkNotInstalled(tabname)
    ...
    xlib.makelabel { x = 10, y = 30, wordwrap = true, w = 365, label = "XGUI 未能在服务端正常加载. 可能是因为服务器更换地图后负载过高, XGUI启动时发生致命错误, 或XGUI未正确安装.", parent = xgui.notInstalledWarning }
    ...
    xlib.makebutton { x = 37, y = 83, w = 80, label = "Offline Mode", parent = xgui.notInstalledWarning }.DoClick = function()
        ...
        xlib.makelabel { x = 10, y = 30, wordwrap = true, w = 365, label = "XGUI将会在离线模式中在本地运行. 一些功能将不可用, 一些信息将会不可视. 你可以使用 '刷新服务器数据' 按钮来尝试重新连接至服务器.", parent = offlineWarning }
    ...
    xlib.makebutton { x = 147, y = 83, w = 80, label = "重试", parent = xgui.notInstalledWarning }.DoClick = function()
        xgui.notInstalledWarning:Remove()
        xgui.notInstalledWarning = nil
        RememberCursorPosition()
        gui.EnableScreenClicker(false)
        local reattempt = xlib.makeframe { label = "XGUI: 尝试连接至服务器...", w = 200, h = 20, nopopup = true, showclose = false, skin = xgui.settings.skin }
...
function xgui.show(tabname)
    ...
    if not game.SinglePlayer() and not ULib.ucl.authed[LocalPlayer():UniqueID()] then
        ...
        xlib.makelabel { label = "你的 ULX player 未被验证!", x = 10, y = 30, parent = unauthedWarning }
        xlib.makelabel { label = "请稍等片刻后重试.", x = 10, y = 45, parent = unauthedWarning }
        xlib.makebutton { x = 50, y = 63, w = 60, label = "重试", parent = unauthedWarning }.DoClick = function()
...
function xgui.expectChunks(numofchunks)
    if xgui.isInstalled then
        ...
        xgui.chunkbox.Label:SetText("获取数据:等待服务器响应...")
